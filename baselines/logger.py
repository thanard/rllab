...
def add_log_output_format(log_output_formats):
    LOG_OUTPUT_FORMATS.extend(log_output_formats)
...
class CSVOutputFormat(OutputFormat):
    ...
    def writekvs(self, kvs):
        if not self.is_head:
            self.file.write(",".join(sorted(kvs.keys())) + '\n')
            self.is_head = True
        values = []
        for k, v in sorted(kvs.items()):
            if hasattr(v, 'dtype'):
...
def make_output_format(format, ev_dir):
    ...
    if format == 'stdout':
        ...
        csv_file = open(osp.join(ev_dir, 'progress.csv'), 'wt')
...
def save_params(kvs, dir):
    with open(os.path.join(dir,"params.json"), 'wt') as f:
        json.dump(kvs, f, sort_keys=True, indent=4, separators=(',', ': '))

def configure(dir=None, format_strs=None, prefix="openai"):
    ...
    if dir is None:
        dir = os.path.join(os.getenv('OPENAI_LOGDIR'),
                           prefix,
                           prefix + datetime.datetime.now().strftime("_%Y_%m_%d_%H_%M_%S"))
    if dir is None:
...
configure(rllab_logger.get_snapshot_dir())
